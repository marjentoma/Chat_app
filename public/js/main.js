...
function outputMessage(message) {
    ...
    if(users == username) {
        $('.message').append(`<p class="outputM1"><span id="txt" >${message.text}</span><small class='outputM text-primary' ><br>${message.username}</small>&nbsp;<small class='text-muted'>${message.time}</small></p>`)
 } 
else {
        $('.message').append(`<p><span id="txt1">${message.text}</span><small class='type text-primary'><br>${message.username}&nbsp;<span class='text-muted'>${message.time}</span></small></p>`)
    }
}
